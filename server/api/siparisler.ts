...
function getSiparislerList(connection: mysql.Connection) {
  return new Promise((resolve, reject) => {
    connection.query("USE Restoran", error => {
      ...
      connection.query(`
        SELECT Siparis.siparis_id AS siparis_id, Siparis.masa_id AS masa_id, Siparis.adet AS adet, Yiyecek.ad AS ad, Yiyecek.fiyat AS fiyat, YiyecekTur.ad AS tur, Siparis.siparis_tarih AS tarih, Yiyecek.fotograf AS fotograf
        FROM Siparis
